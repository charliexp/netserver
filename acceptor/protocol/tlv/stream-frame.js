...
StreamFrame.prototype.handleData = function (buff) {
    ...
    do{  
        if( this.pending.length >= ping.length )
        {        
            if( head && (head.readUInt16BE(0) === this.pending.readUInt16BE(0)) )
            {              
                break;                                    
            }
            else if( ping&&(ping.data.readUInt16BE(0) === this.pending.readUInt16BE(0) ) )
            {
                this.emit( 'ping', ping.data );    
